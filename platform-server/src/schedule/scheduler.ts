...
async function runResume(schedule: Schedule): Promise<void> {
  ...
  if (isStreaming(schedule.sessionId)) {
    ...
    markScheduleRun(schedule.id, {})
    auditWrite('platform', 'schedule not needed', schedule.title, 'read', 'OK')
    return
  }
  ...
  if (refusal) {
    ...
    auditWrite('platform', 'schedule refused', `${schedule.title}: ${refusal}`.slice(0, 120), 'write', 'denied')
    return
  }
  ...
  auditWrite('platform', 'schedule resumed a conversation', schedule.title, 'write', 'OK')
...
async function runRecurring(schedule: Schedule, now: number): Promise<void> {
  ...
  auditWrite('platform', 'schedule started a conversation', schedule.title, 'write', 'OK')
