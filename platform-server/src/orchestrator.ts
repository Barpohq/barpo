...
export async function javobOqizi(
  ...
  try {
    const oqim = sozlama?.toollar === false
      ? suhbatOqimi(tanlov, tarix, { signal: boshqaruv.signal })
      : agentOqimi(tanlov, tarix, {
          sessionId,
          ishPapkasi: ishPapkasi(sessionId),
      ...
      switch (hodisa.tur) {
        ...
        case 'rejim':
          hub.broadcast({
            type: 'chat.rejim',
            sessionId,
            holat: { rejim: hodisa.rejim, sabab: hodisa.sabab, klassifikatorModeli: klassifikatorNomi() },
          })
          if (hodisa.sabab) {
  ...
  if (saqlanadigan.length > 0 || toolCards.length > 0) {
    xabarYoz({ id: messageId, sessionId, role: 'assistant', text: saqlanadigan, toolCards })
  }
 ...
 */
function klassifikatorNomi(): string | undefined {
  const tanlov = klassifikatorModeliniTanla(keshdagiNatija()?.models ?? [])
...
export function rejimHolati(sessionId: string): RejimHolati {
  ...
  return { ...b.holat, klassifikatorModeli: klassifikatorNomi() }
}
 ...
 * Bo'sh matnli xabarlar tashlab yuboriladi — ba'zi providerlar ularni rad etadi.
 */
function tarixniTayyorla(sessionId: string): SuhbatXabari[] {
