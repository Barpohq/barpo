...
import { claudeCodeAuth, codexAuth, mahalliyAuthlar } from '../src/mahalliy-auth.ts'
import { modelTartibi } from '../src/aniqlash.ts'
...
describe('mahalliy OAuth manba nomi', () => {
  ...
  test('har bir topilma provider id va manba juftini beradi', async () => {
    ...
    const natijalar = await mahalliyAuthlar(uy)
...
function model(qism: Partial<ModelInfo> & { name: string; manbaTuri: ManbaTuri }): ModelInfo {
  return {
