...
import { REESTR_CHEGARASI, REESTR_TTL_MS, SessiyaReestri } from '../src/reestr.ts'
...
function reestrYarat(ttlMs = REESTR_TTL_MS, chegara = REESTR_CHEGARASI) {
  return new SessiyaReestri<SoxtaBoshqaruvchi>(
...
describe('LRU — soni bo\'yicha chegara', () => {
  test('chegaradan oshsa eng eskisi chiqariladi', () => {
    const r = reestrYarat(REESTR_TTL_MS, 3)
    const a = r.ol('s1', 0)
    r.ol('s2', 1)
    r.ol('s3', 2)
  ...
  test('chegara qat\'iy ushlab turadi (ko\'p sessiya anomaliyasi)', () => {
    const r = reestrYarat(REESTR_TTL_MS, 10)
...
describe('mustahkamlik', () => {
  test('yop() xatosi tozalashni to\'xtatmaydi', () => {
    const r = new SessiyaReestri<{ yop(): void }>(
      () => ({
        yop() {
  ...
  test('standart qiymatlar oqilona', () => {
    ...
    expect(REESTR_TTL_MS).toBeGreaterThanOrEqual(5 * 60 * 1000)
    expect(REESTR_CHEGARASI).toBeGreaterThan(0)
  })
})
