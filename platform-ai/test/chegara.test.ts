...
import { chegaraMi, chegaralarniAjrat } from '../src/chegara.ts'

describe('chegaraMi — haqiqiy chegaralar ushlanadi', () => {
  const chegaralar = [
  ...
  for (const matn of chegaralar) {
    test(`chegara: ${matn}`, () => {
      expect(chegaraMi(matn)).toBe(true)
    })
  }
})

describe('chegaraMi — oddiy so\'rovlar chegara emas', () => {
  ...
  for (const matn of oddiylar) {
    test(`oddiy: ${matn}`, () => {
      expect(chegaraMi(matn)).toBe(false)
    })
  }
})

describe('chegaralarniAjrat', () => {
  test('faqat foydalanuvchi xabarlari olinadi', () => {
    const natija = chegaralarniAjrat([
