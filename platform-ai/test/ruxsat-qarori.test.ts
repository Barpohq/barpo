...
// `'ruxsat' | 'rad'` qaytarardi, ya'ni auto rejim ruxsat berdimi,
...
import type { RuxsatQarori } from '@platforma/shared'
...
let boshqaruv: RuxsatBoshqaruvchi
let qarorlar: RuxsatQarori[]

beforeEach(() => {
  boshqaruv = new RuxsatBoshqaruvchi('sinov-sessiya')
  qarorlar = []
...
describe('ruxsat qarori manbasi', () => {
  test("foydalanuvchi ruxsat berdi — manba 'foydalanuvchi'", async () => {
    ...
    boshqaruv.javobBer(sorov.id, 'ruxsat')

    expect(await kutilmoqda).toBe('ruxsat')
  ...
  test('"har doim" ikki xil manba beradi: bosilgani va keyingi ishlashi', async () => {
    ...
    boshqaruv.javobBer(boshqaruv.kutayotganSorovlar[0]!.id, 'hardoim')
    expect(await birinchi).toBe('ruxsat')
    ...
    expect(await boshqaruv.sora(sorash())).toBe('ruxsat')
    expect(qarorlar).toHaveLength(2)
    expect(qarorlar[1]).toMatchObject({ manba: 'hardoim', berildi: true, naqsh: 'bash:ls' })
    expect(qarorlar[1]!.sorovId).toBeUndefined()
  })

  test("rad etish yozib olinadi", async () => {
    const kutilmoqda = boshqaruv.sora(sorash())
    boshqaruv.javobBer(boshqaruv.kutayotganSorovlar[0]!.id, 'rad')

    expect(await kutilmoqda).toBe('rad')
    expect(qarorlar[0]).toMatchObject({ manba: 'rad', berildi: false })
  })

  test("muddat tugashi ham qaror — jimgina yo'qolmaydi", async () => {
    boshqaruv.kutishMuddatiniOrnat(10)
    expect(await boshqaruv.sora(sorash())).toBe('rad')
  ...
  test("oqim bekor qilinsa so'rov DARHOL yopiladi", async () => {
    ...
    const qaytar: RuxsatQarori[] = []
    const b = new RuxsatBoshqaruvchi('bekor-sessiya')
  ...
  test('allaqachon bekor qilingan oqimda so\'rov umuman ro\'yxatga tushmaydi', async () => {
    const boshqaruvchi = new AbortController()
    boshqaruvchi.abort()
    const b = new RuxsatBoshqaruvchi('bekor-2')
    const qaytar: RuxsatQarori[] = []
    b.ruxsatQarorlariniKuzat((q) => qaytar.push(q))
    b.klassifikatorniUla({
      rejim: { rejim: 'tasdiq' } as never,
  ...
  test('javob berilgan so\'rov keyin bekor qilinsa IKKINCHI qaror yozilmaydi', async () => {
    const boshqaruvchi = new AbortController()
    const b = new RuxsatBoshqaruvchi('bekor-3')
    const qaytar: RuxsatQarori[] = []
    b.ruxsatQarorlariniKuzat((q) => qaytar.push(q))
    b.klassifikatorniUla({
      rejim: { rejim: 'tasdiq' } as never,
    ...
    b.javobBer(b.kutayotganSorovlar[0]!.id, 'ruxsat')
    expect(await kutilmoqda).toBe('ruxsat')
  ...
  test("kuzatuvchi xatosi ruxsat oqimini buzmaydi", async () => {
    ...
    const kutilmoqda = boshqaruv.sora(sorash())
    boshqaruv.javobBer(boshqaruv.kutayotganSorovlar[0]!.id, 'ruxsat')
    expect(await kutilmoqda).toBe('ruxsat')
  })
})
