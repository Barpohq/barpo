// `.platforma` papkasi qidiruvdan chiqarilganini tekshiradi.
//
// NEGA MUHIM. Loyihaga ulangan suhbatlar bitta ish papkasini bo'lishadi
// (`ish-papkasi.ts`), biriktirmalar esa `.platforma/sessiyalar/<sid>/fayllar/`
// da yashaydi. Papka qidiruvdan chiqarilmasa, agent `grep` qilganda BOSHQA
// suhbatlarning fayllaridan natija chiqardi — suhbatlar orasida ma'lumot
// sizishi.
//
// Ikkinchi shart ham xuddi shunday muhim: ANIQ YO'L berilsa fayl baribir
// ko'rinishi kerak. Biriktirma oqimi shunga tayanadi — agent promptdagi
// yo'lni `read` ga beradi va o'qishi shart.
//
// Test HAQIQIY fayl tizimida ishlaydi (vaqtinchalik papkada): filtr
// `TASHLANADIGAN_PAPKALAR` ro'yxatida ham, ikkala backendda ham
// (`rg` va Node zaxirasi) qo'llanishi kerak, ya'ni soxta fayl tizimi bu
// himoyani tekshirmagan bo'lardi.

import { afterEach, beforeEach, describe, expect, test } from 'bun:test'
import { mkdirSync, mkdtempSync, rmSync, writeFileSync } from 'node:fs'
import { tmpdir } from 'node:os'
import { join } from 'node:path'
import { ChegaralanganMuhit } from '../src/muhit.ts'
import { TASHLANADIGAN_PAPKALAR } from '../src/qidiruv-asos.ts'
import { qidiruvToollariXom } from '../src/qidiruv-toollari.ts'
import { RuxsatBoshqaruvchi } from '../src/ruxsat.ts'

let papka: string
let kontekst: { env: ChegaralanganMuhit }

/** Begona sessiyaning biriktirma papkasi */
const BEGONA_YOL = '.platforma/sessiyalar/boshqa-sid/fayllar'

beforeEach(() => {
  papka = mkdtempSync(join(tmpdir(), 'platforma-qidiruv-biriktirma-'))

  // Oddiy kod fayli — topilishi KERAK
  writeFileSync(join(papka, 'kod.ts'), 'const BELGI = "kod ichida"\n')

  // Begona sessiyaning biriktirmasi — topilmasligi kerak
  mkdirSync(join(papka, BEGONA_YOL), { recursive: true })
  writeFileSync(join(papka, BEGONA_YOL, 'begona.txt'), 'const BELGI = "begona suhbatdan"\n')

  // Skill va xotira ham shu papkada — ular ham chiqadi (ongli almashtirish)
  mkdirSync(join(papka, '.platforma/memory'), { recursive: true })
  writeFileSync(join(papka, '.platforma/memory/eslatma.md'), 'const BELGI = "xotirada"\n')

  // Ruxsat so'ralishi bu testlarda KUTILMAYDI: hamma yo'l ish papkasi
  // ichida. So'rov kelsa u javobsiz qoladi va test timeout bilan yiqiladi —
  // bu to'g'ri xulq, chunki chegara buzilganini bildiradi.
  kontekst = {
    env: new ChegaralanganMuhit({
      ishPapkasi: papka,
      ruxsat: new RuxsatBoshqaruvchi('sinov'),
    }),
  }
})

afterEach(() => {
  rmSync(papka, { recursive: true, force: true })
})

/** Tool natijasidagi matn */
async function bajar(nom: string, args: unknown): Promise<string> {
  const tool = qidiruvToollariXom().find((t) => t.name === nom)
  if (!tool) throw new Error(`tool topilmadi: ${nom}`)
  const natija = await (
    tool.execute as unknown as (
      id: string,
      p: unknown,
      s: undefined,
      u: undefined,
      k: unknown,
    ) => Promise<{ content: { type: string; text?: string }[] }>
  )('t1', args, undefined, undefined, kontekst)
  return natija.content
    .filter((c) => c.type === 'text')
    .map((c) => c.text ?? '')
    .join('\n')
}

describe('`.platforma` qidiruvdan chiqarilgan', () => {
  test('ro\'yxatda bor — ikkala backend shu ro\'yxatdan o\'qiydi', () => {
    expect(TASHLANADIGAN_PAPKALAR).toContain('.platforma')
  })

  test('grep begona sessiyaning biriktirmasini TOPMAYDI', async () => {
    const natija = await bajar('grep', { pattern: 'BELGI' })

    expect(natija).toContain('kod.ts')
    expect(natija).not.toContain('begona.txt')
  })

  test('find begona biriktirmani ko\'rsatmaydi', async () => {
    const natija = await bajar('find', { pattern: '*.txt' })

    expect(natija).not.toContain('begona.txt')
  })

  test('ls ildizda `.platforma` ni ko\'rsatmaydi', async () => {
    const natija = await bajar('ls', {})

    expect(natija).toContain('kod.ts')
    expect(natija).not.toContain('.platforma')
  })

  // ONGLI YON TA'SIR: xotira va skilllar ham qidiruvdan chiqadi. Ular
  // promptga baribir to'liq tushadi (`xotira.ts`, `skill-yuklash.ts`), ya'ni
  // agent ularni ko'radi — faqat `grep` bilan izlay olmaydi. Bu test o'zgarish
  // ONGLI ekanini yozib qo'yadi: kimdir buni "xato" deb tuzatmoqchi bo'lsa,
  // yuqoridagi izohni o'qishi kerak.
  test('xotira fayllari ham qidiruvdan chiqadi (ongli almashtirish)', async () => {
    const natija = await bajar('grep', { pattern: 'BELGI' })

    expect(natija).not.toContain('eslatma.md')
  })
})

describe('aniq yo\'l bilan o\'qish ishlaydi', () => {
  // BIRIKTIRMA OQIMI SHUNGA TAYANADI: agent promptdagi yo'lni `read` ga
  // beradi. Ro'yxat faqat AYLANIB CHIQISHNI to'sadi, aniq so'rovni emas.
  test('ls aniq yo\'l bilan biriktirmalarni ko\'rsatadi', async () => {
    const natija = await bajar('ls', { path: BEGONA_YOL })

    expect(natija).toContain('begona.txt')
  })

  test('grep aniq papkada izlaydi', async () => {
    const natija = await bajar('grep', { pattern: 'BELGI', path: BEGONA_YOL })

    expect(natija).toContain('begona.txt')
  })
})
