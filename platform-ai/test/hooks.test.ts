...
describe('oldin zanjiri', () => {
  test('hech kim bloklamasa undefined', async () => {
    const hooklar: ToolHook[] = [{ nom: 'a' }, { nom: 'b', oldin: () => undefined }]
    expect(await beforeChain(hooklar, kontekst)).toBeUndefined()
  })

  test('bloklagan hook toolni to\'xtatadi', async () => {
    const hooklar: ToolHook[] = [{ nom: 'a', oldin: () => ({ blokla: true, sabab: 'yaramaydi' }) }]
    const n = await beforeChain(hooklar, kontekst)
  ...
  test('async hook qo\'llab-quvvatlanadi', async () => {
    ...
    expect((await beforeChain(hooklar, kontekst))?.sabab).toBe('async blok')
  })

  test('sabab berilmasa hook nomi ishlatiladi', async () => {
    const hooklar: ToolHook[] = [{ nom: 'nomsiz', oldin: () => ({ blokla: true }) }]
    expect((await beforeChain(hooklar, kontekst))?.sabab).toContain('nomsiz')
  })
})

describe('keyin zanjiri', () => {
  test('hech kim o\'zgartirmasa natija o\'zgarmaydi', async () => {
    const n = await afterChain([{ nom: 'a' }], natijaKonteksti)
    expect(n.natija).toBe('chiqish')
    expect(n.xatomi).toBe(false)
  })

  test('hook natijani almashtiradi', async () => {
    const hooklar: ToolHook[] = [{ nom: 'a', keyin: () => ({ natija: 'yangi' }) }]
    expect((await afterChain(hooklar, natijaKonteksti)).natija).toBe('yangi')
  })
  ...
  test('hook\'lar ZANJIR bo\'lib ishlaydi — keyingisi oldingisining natijasini ko\'radi', async () => {
    ...
    expect((await afterChain(hooklar, natijaKonteksti)).natija).toBe('chiqish-1-2')
  })
  ...
  test('hook xatosi natijani yo\'qotmaydi, lekin ko\'rinadi', async () => {
    ...
    const n = await afterChain(hooklar, natijaKonteksti)
    expect(n.natija).toContain('chiqish')
    expect(n.natija).toContain('buzuq')
  })

  test('xato bayrog\'ini o\'zgartirish mumkin', async () => {
    const hooklar: ToolHook[] = [{ nom: 'a', keyin: () => ({ xatomi: true }) }]
    expect((await afterChain(hooklar, natijaKonteksti)).xatomi).toBe(true)
  })
})
...
describe('maxfiyni yashirish', () => {
  ...
  async function yashir(natija: string): Promise<string> {
    return (await afterChain([hook], { ...natijaKonteksti, natija })).natija
  }
...
describe('uzunlik hooki', () => {
  test('uzun natija kesiladi va bu aytiladi', async () => {
    ...
    const n = await afterChain([lengthHook(100)], { ...natijaKonteksti, natija: uzun })
    expect(n.natija.length).toBeLessThan(200)
    expect(n.natija).toContain('truncated')
  })

  test('qisqa natija tegilmaydi', async () => {
    const n = await afterChain([lengthHook(100)], natijaKonteksti)
    expect(n.natija).toBe('chiqish')
  })
})
...
describe('qo\'shimcha taqiq', () => {
  test('taqiqlangan buyruq bloklanadi', async () => {
    ...
    const n = await beforeChain([hook], { ...kontekst, args: { command: 'docker ps' } })
    expect(n?.blokla).toBe(true)
  ...
  test('to\'liq yo\'l bilan yashirish ushlanadi', async () => {
    const hook = extraDenyHook(['docker'])
    const n = await beforeChain([hook], { ...kontekst, args: { command: '/usr/bin/docker ps' } })
    expect(n?.blokla).toBe(true)
  })

  test('zanjir ichida yashirilgan buyruq ushlanadi', async () => {
    const hook = extraDenyHook(['docker'])
    const n = await beforeChain([hook], { ...kontekst, args: { command: 'ls && docker ps' } })
    expect(n?.blokla).toBe(true)
  })

  test('taqiqlanmagan buyruq o\'tadi', async () => {
    const hook = extraDenyHook(['docker'])
    expect(await beforeChain([hook], { ...kontekst, args: { command: 'ls -la' } })).toBeUndefined()
  })

  test('bo\'sh taqiq ro\'yxati hech narsani bloklamaydi', async () => {
    const hook = extraDenyHook([])
    expect(await beforeChain([hook], kontekst)).toBeUndefined()
  })

  test('bash bo\'lmagan tool tegilmaydi', async () => {
    const hook = extraDenyHook(['docker'])
    const n = await beforeChain([hook], { nom: 'read', args: { path: 'docker' }, workDir: '/i', sessionId: 's' })
...
describe('kuzatuv hooki', () => {
  test('chaqiriladi va bloklamaydi', async () => {
    ...
    const hook = observerHook((k) => korilgan.push(k.nom))
    expect(await beforeChain([hook], kontekst)).toBeUndefined()
