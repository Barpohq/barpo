...
// Generate with: `bun run barpo-config/src/schema-write.ts`.
...
export const FIELDS = [
  ...
  // --- Session ---
  {
    ...
    default: null,
    nullable: true,
    hint: 'Root directory the agent tools work in. When `null`, `~/.barpo/work/` is used.',
  },
  {
  ...
  // comment in `barpo-ai/src/mcp-tools.ts`). With a flag the user could
...
export interface Config {
  ...
  session: {
    /** Directory path, or null (`~/.barpo/work/`) */
    workDir: string | null
    idleMinutes: number
  }
  mcp: {
